...
def evaluate_convnet(data_path, n_cand_chunk, base_lr=0.1, stepsize=50000, gamma = 0.5, momentum=0.0,
                     n_epochs= 10000,
                     nkerns=[20, 50], n_hidden=50, batch_size=50, isDropout=False,
                     N_valid = 100000, N_test = 100000,
                     validate_every_batches = 100, n_rot = 0, activation = T.tanh,
    ...
    maxpool_size1 = int(math.ceil(1.0*(im_size-filter_shape1 + 1)/pool_size))
    ...
    maxpool_size2 = int(math.ceil(1.0*(maxpool_size1-filter_shape2 + 1)/pool_size2))

    print "output= ", maxpool_size2

    print "hidden layer inputs", nkerns[1] * maxpool_size2 * maxpool_size2
