...
def evaluate_convnet(data_path, n_cand_chunk, base_lr=0.1, stepsize=50000, gamma = 0.5, momentum=0.0,
    ...
    maxpool_size3 = (maxpool_size2+filter_shape3 - 1)/pool_stride3
    ...
    layer3 = HiddenLayer(
        rng,
        input=layer3_input,
        n_in=nkerns[2] * maxpool_size3 * maxpool_size3,
        n_out=nkerns[3],
        activation=activation
        #activation=T.tanh
        #activation=relu
    ...
    if isDropout:
        print "Dropout ON"
        drop_layer4 = DropoutLayer(layer4.output, p_drop=0.5)    
        # classify the values of the fully-connected sigmoidal layer
        layer5 = LogisticRegression(input=drop_layer4.output, n_in=nkerns[4], n_out=2)
    else:
        print "Dropout OFF"
        layer5 = LogisticRegression(input=layer4.output, n_in=nkerns[4], n_out=2)
    ...
    predict = theano.function([index],layer5.p_y_given_x,
